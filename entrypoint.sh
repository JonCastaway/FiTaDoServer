...
log_message() {
    echo "$(date '+%d-%m-%Y %H:%M:%S') - $1" | tee -a /var/log/DockerComposeFFsync_setup.log
}

# Run migrations
log_message "Running migrations for sync storage..."
if /usr/local/cargo/bin/diesel --database-url "${SYNC_SYNCSTORAGE_DATABASE_URL}" migration --migration-dir syncstorage-mysql/migrations run; then
    log_message "Sync storage migration successful."
else
    log_message "Sync storage migration failed."
    exit 1
fi

log_message "Running migrations for token server..."
if /usr/local/cargo/bin/diesel --database-url "${SYNC_TOKENSERVER_DATABASE_URL}" migration --migration-dir tokenserver-db/migrations run; then
    log_message "Token server migration successful."
else
    log_message "Token server migration failed."
    exit 1
fi
